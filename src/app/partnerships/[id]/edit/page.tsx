...
export default function EditPartnership() {
  ...
  const { getPartnershipById, updatePartnership } = usePartnerships();
  ...
  }, [id, getPartnershipById]);

  const handleAddItem = (itemId: string, itemName: string, type: 'product' | 'service' | 'box') => {
    if (!selectedItems.find(item => item.id === itemId && item.type === type)) {
      setSelectedItems([...selectedItems, { id: itemId, name: itemName, type, partnerPrice: '' }]);
    }
  };

  const handleRemoveItem = (itemId: string, type: 'product' | 'service' | 'box') => {
    setSelectedItems(selectedItems.filter(item => !(item.id === itemId && item.type === type)));
  };

  const handleUpdatePrice = (itemId: string, type: 'product' | 'service' | 'box', price: string) => {
    setSelectedItems(selectedItems.map(item => 
      item.id === itemId && item.type === type ? { ...item, partnerPrice: price } : item
    ));
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (name && selectedItems.length > 0 && selectedItems.every(item => item.partnerPrice) && id) {
  ...
  return (
    <div className="container mx-auto p-8 max-w-4xl">
      ...
      <h1 className="text-4xl font-bold mb-8 text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-orange-600">✏️ Editar Parceria</h1>
      
      {submitted && (
        <div className="bg-green-900/30 border border-green-600 text-green-400 px-4 py-3 rounded-lg mb-6 flex items-center gap-2">
          <span>✅</span> Parceria atualizada com sucesso!
        </div>
      )}
