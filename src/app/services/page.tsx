  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      <div className="flex justify-between items-center mb-6 gap-4 flex-wrap">
        <h1 className="text-2xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-orange-600">⚙️ Serviços</h1>
        <Link
      ...
      {services.length === 0 ? (
        <div className="text-center py-16">
          <p className="text-slate-400 text-xl mb-6">📭 Nenhum serviço cadastrado</p>
          <Link
            href="/services/register"
              ...
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
                <div className="flex items-start gap-4 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="text-xl md:text-2xl font-bold text-white mb-4">{service.name}</h3>
                    <div className="bg-slate-700 p-4 rounded-lg inline-block">
                      <p className="text-slate-400 text-xs uppercase mb-1">Valor do Serviço</p>
                      <p className="text-2xl font-bold text-orange-400">R$ {formatBRLWithoutCurrency(service.price)}</p>
                                        {service.hasDisplacement && (
                                          <div className="mt-3">
                                            <span className="inline-flex items-center gap-1.5 bg-orange-500/20 border border-orange-500/50 text-orange-400 text-xs font-bold px-3 py-1 rounded-full">
                                              🚗 Deslocamento por KM
                                            </span>
                                          </div>
                                        )}
                ...
                <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50">
                  <div className="bg-slate-800 border border-orange-600 rounded-xl p-8 max-w-sm">
                    ...
                    <p className="text-slate-300 mb-6">
                      Tem certeza que deseja deletar o serviço "<strong>{service.name}</strong>"?
                    </p>
