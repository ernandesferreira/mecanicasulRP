  ...
  return (
    <div className="container mx-auto p-4 md:p-8">
      <div className="flex justify-between items-center mb-6 gap-4 flex-wrap">
        <h1 className="text-2xl md:text-4xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-orange-400 to-orange-600">🛠️ Produtos</h1>
        <Link
      ...
      {products.length === 0 ? (
        <div className="text-center py-16">
          <p className="text-slate-400 text-xl mb-6">📭 Nenhum produto cadastrado</p>
          <Link
            href="/products/register"
                ...
                <div className="flex items-start gap-4 flex-1 min-w-0">
                  ...
                  <div className="mt-4 grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
                    <div className="bg-slate-700 p-3 rounded-lg">
                      <p className="text-slate-400 text-xs uppercase">Preço Venda</p>
                      <p className="text-lg font-bold text-orange-400 mt-1">R$ {formatBRLWithoutCurrency(product.price)}</p>
                    </div>
                    <div className="bg-slate-700 p-3 rounded-lg">
                      <p className="text-slate-400 text-xs uppercase">Custo</p>
                      <p className="text-lg font-bold text-slate-300 mt-1">R$ {formatBRLWithoutCurrency(product.cost)}</p>
                    </div>
                    <div className="bg-slate-700 p-3 rounded-lg">
                      <p className="text-slate-400 text-xs uppercase">Parceria</p>
                      <p className="text-lg font-bold text-slate-300 mt-1">R$ {product.partnership ? formatBRLWithoutCurrency(product.partnership) : '-'}</p>
                    </div>
                    <div className="bg-slate-700 p-3 rounded-lg">
                      <p className="text-slate-400 text-xs uppercase">Lucro</p>
                      <p className="text-lg font-bold text-green-400 mt-1">R$ {formatBRLWithoutCurrency(product.price - product.cost)}</p>
                    </div>
                  </div>
                </div>
                </div>
